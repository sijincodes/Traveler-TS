            ...
            {placesAPI?.map((elm, index) => (
              <div key={index} className="flex items-center gap-5 sm:gap-3 rounded-lg hover:bg-emerald-300 transition-all duration-300 cursor-pointer hover:scale-105">
                <div className="flex items-center">
                  <img src={elm.placeImg} alt="place-img" className="w-20 h-20 sm:w-16 sm:h-16 rounded-lg filter drop-shadow-lg" />
                </div>
                <div className=" flex items-start flex-col text-slate-900">
                    <h1 className="text-lg sm:text-sm font-bold">{elm.location}</h1>
                    <p className="font-normal lg:text-sm text-base sm:text-xs">{elm.distance}</p>
                </div>
              </div>
            ))}
          </div>
